        ...
        (() => {
          ...
          if (!window.__image_downloader_injected__) {
            ...
            btn.innerHTML = \`
              ...
              <span style="margin-left: 6px; font-size: 11px; font-family: system-ui, -apple-system, sans-serif; font-weight: bold; opacity: 0; transition: opacity 0.15s ease-out; flex-shrink: 0; pointer-events: none;">下载素材</span>
            \`;
            ...
            document.addEventListener('mouseover', (e) => {
              ...
              activeMetadata = {
                url: src,
                title: img.alt || img.title || document.title || '提取素材',
                width: w,
                height: h
              };
          ...
          if (!window.__image_hover_zoom_injected__) {
            ...
            function showPreview(el, src) {
              ...
              const altText = el.alt || el.title || document.title || '高清图片预览';
              previewImg.src = src;
              previewImg.style.maxWidth = '100%';
              previewImg.style.maxHeight = '100%';
              ...
              if (lastSentAssetUrl !== currentHighResUrl && window.browserAPI && window.browserAPI.setHoveredAsset) {
                ...
                window.browserAPI.setHoveredAsset({
                  ...
                  title: el.alt || el.title || document.title || '提取素材',
              ...
              preloadImageObj.onerror = () => {
                if (fallbackUrl && activeRequestUrl === hiResUrl) {
                  ...
                  spinner.style.display = 'none';
                  sizeEl.textContent = (previewImg.naturalWidth || '?') + ' x ' + (previewImg.naturalHeight || '?') + ' (加载原图失败)';
                }
              };
            ...
            window.addEventListener('keydown', (e) => {
              ...
              if ((e.key === 's' || e.key === 'S' || e.code === 'KeyS') && activeElement && previewContainer.style.display === 'flex' && window.browserAPI) {
                ...
                const title = titleEl.textContent || '提取素材';
                ...
                if (url && !url.startsWith('data:')) {
                  ...
                  // Premium tactile feedback: flash info bar size to show "⚡ 已启动原图下载..."
                  const prevSizeText = sizeEl.textContent;
                  sizeEl.textContent = '⚡ 已启动原图下载...';
