...
export class PaddleOcrColorTextBoxProvider implements ITextBoxProvider {
  ...
  public async detect(imagePath: string, _assetId?: string): Promise<TextBox[]> {
    ...
    const workerPath = path.resolve(workspace, 'tools', 'ocr', 'paddleocr_detect.py')

    console.log(`[PaddleOcrColorTextBoxProvider] Spawning PaddleOCR worker: ${workerPath} for image: ${imagePath} using ${pythonExe}`)

    if (!fs.existsSync(workerPath)) {
      throw new Error(`PaddleOCR worker script not found at: ${workerPath}`)
    }
    ...
    return new Promise<TextBox[]>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(new Error(`PaddleOCR worker process execution timed out after ${timeoutMs}ms.`))
      }, timeoutMs)
      ...
      child.on('error', (err) => {
        if (killed) return
        clearTimeout(timer)
        reject(new Error(`Failed to spawn PaddleOCR worker process: ${err.message}`))
      })
      ...
      child.on('close', (code) => {
        ...
        if (code !== 0 || !trimmedStdout) {
          ...
          reject(new Error(`PaddleOCR worker process exited with code ${code}.${stderrSummary}`))
          return
        }
        ...
        try {
          ...
          if (parsed.success === false && parsed.error) {
            reject(new Error(`PaddleOCR worker error: [${parsed.error.code}] ${parsed.error.message}\nStderr:\n${parsed.error.stderr || stderrData}`))
            return
          }
          ...
          if (parsed.success === true && parsed.data && Array.isArray(parsed.data.items)) {
            ...
            reject(new Error(`Invalid structured JSON response returned from PaddleOCR worker. stdout:\n${trimmedStdout}`))
          }
        } catch (jsonErr: any) {
          reject(new Error(`Failed to parse structured JSON from PaddleOCR worker: ${jsonErr.message}. Raw stdout:\n${trimmedStdout}\nStderr:\n${stderrData}`))
        }
      })
    })
  }
