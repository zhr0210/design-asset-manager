...
| Qwen3-VL MLX | Planned capability. | `macos_ai_capabilities.py` labels MLX route planned/fallback based on platform/import. | Looks like a product route but has no complete model install/run path yet. | Hide or mark explicitly as “planned, not installed.” |
