...
class TestPythonCudaCompat(unittest.TestCase):
    ...
    def test_cuda_ready_reports_optional_status(self):
        ...
        result = probe_python_cuda_environment(
        ...
        self.assertTrue(result["diagnostics"]["torch"]["cudaBuilt"])
        self.assertTrue(result["diagnostics"]["torch"]["cudaAvailable"])
        self.assertTrue(result["diagnostics"]["families"]["ram"]["available"])

    def test_missing_torch_reports_environment_insufficient(self):
        result = probe_python_cuda_environment(import_module=self.fake_import({}))
        self.assertFalse(result["compatible"])
        self.assertEqual(result["error"]["code"], "ENVIRONMENT_INSUFFICIENT")
        self.assertEqual(result["status"], "unavailable")

    def test_cuda_unavailable_reports_cuda_unavailable(self):
        torch = types.SimpleNamespace(
            __version__="2.6.0",
            version=types.SimpleNamespace(cuda="12.4"),
        ...
        self.assertFalse(result["compatible"])
        self.assertTrue(result["diagnostics"]["torch"]["cudaBuilt"])
        self.assertFalse(result["diagnostics"]["torch"]["cudaAvailable"])
        self.assertEqual(result["error"]["code"], "CUDA_UNAVAILABLE")
