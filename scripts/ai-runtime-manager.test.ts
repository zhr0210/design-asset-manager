...
assert.deepEqual(profiles.find((profile) => profile.id === 'macos-apple-silicon')?.recommendedRuntimeKinds, ['python-worker', 'llama-app'])
assert.deepEqual(profiles.find((profile) => profile.id === 'macos-intel')?.recommendedRuntimeKinds, ['custom-http'])
