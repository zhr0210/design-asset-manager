...
const hostContextSource = await fs.readFile('src/main/services/ai-runtime/ai-runtime-host-context.ts', 'utf8')
...
assert.match(hostContextSource, /export function createAiRuntimeHostContext/)
assert.match(handlerSource, /const aiRuntimeHostContext = createAiRuntimeHostContext\(\)/)
assert.match(handlerSource, /bootstrapAiRuntimeManager\(\{/)
assert.match(handlerSource, /platform: aiRuntimeHostContext\.platform/)
assert.match(handlerSource, /arch: aiRuntimeHostContext\.arch/)
assert.match(handlerSource, /homeDir: aiRuntimeHostContext\.homeDir/)
assert.match(handlerSource, /currentPlatform: aiRuntimeHostContext\.platform/)
assert.doesNotMatch(handlerSource, /process\.platform|process\.arch|os\.homedir/)
...
assert.match(bootstrapSource, /runtimeProfileRuleMatchesTarget\(candidate, \{\s*platform: currentPlatform,\s*arch: currentArch\s*\}\)/)
...
assert.doesNotMatch(bootstrapSource, /candidate\.platform === currentPlatform|candidate\.arch === currentArch/)
assert.match(bootstrapSource, /const AI_RUNTIME_BOOTSTRAP_PLATFORM_ADAPTERS: AiRuntimeBootstrapPlatformAdapter\[\]/)
assert.match(bootstrapSource, /resolveAiRuntimeBootstrapPlatformAdapter/)
assert.match(bootstrapSource, /platformAdapterMatchesCurrentPlatform\(candidate, \{ currentPlatform: platform \}\)/)
assert.match(bootstrapSource, /autoStartPythonWorker: true[\s\S]*platform: 'darwin'[\s\S]*autoStartPythonWorker: true[\s\S]*autoStartPythonWorker: false/)
assert.match(bootstrapSource, /if \(!platformAdapter\.autoStartPythonWorker\)/)
assert.doesNotMatch(bootstrapSource, /!candidate\.platform \|\| candidate\.platform === platform/)
assert.doesNotMatch(bootstrapSource, /host\.platform === 'darwin' \|\| host\.platform === 'win32'/)
assert.doesNotMatch(handlerSource, /PLATFORM_AI_BRANCH_RUNTIME_PROVIDER_DESCRIPTORS|AI_RUNTIME_BOOTSTRAP_PLATFORM_ADAPTERS/)
