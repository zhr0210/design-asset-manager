...
assert.equal(unknownGpu.healthLabel, '未知')
...
assert.equal(safeGpu.healthLabel, '正常')
...
assert.equal(highUsageGpu.healthLabel, '关注')
...
assert.equal(lowFreeGpu.healthLabel, '未知')
...
assert.match(overviewWorkflowSource, /AI_CONSOLE_GPU_RISK_DISPLAY/)
assert.match(overviewWorkflowSource, /healthLabel: '正常'/)
assert.match(overviewWorkflowSource, /healthLabel: '关注'/)
assert.match(overviewWorkflowSource, /healthLabel: '未知'/)
...
assert.doesNotMatch(overviewWorkflowSource, /riskTone === 'good' \? '安全'|riskTone === 'bad' \? '高负载'|riskTone === 'bad' \? 'bg-rose-500'/)
...
assert.match(overviewSlice, /props\.gpuDisplay\.healthLabel/)
...
assert.doesNotMatch(overviewSlice, /riskTone === 'good' \? '正常'|riskTone === 'bad' \? '关注'/)
assert.doesNotMatch(aiConsoleSource, /gpuDisplay: ReturnType<typeof projectAiConsoleGpuDisplay>/)
assert.doesNotMatch(aiConsoleSource, /modelsList: any\[\]/)
assert.doesNotMatch(aiConsoleSource, /installedNativeModels: any\[\]/)
assert.doesNotMatch(aiConsoleSource, /nativeModels: any\[\]/)
assert.doesNotMatch(aiConsoleSource, /loadedModels: Record<string, any>/)
