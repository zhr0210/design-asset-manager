...
assert.match(ocrDependencySource, /const OCR_BASE_PYTHON_RESOLVERS: OcrBasePythonResolver\[\]/)
assert.match(ocrDependencySource, /platform: 'win32'[\s\S]*resolve: resolveWindowsBasePythonExecutable/)
assert.match(ocrDependencySource, /platform: 'darwin'[\s\S]*resolve: resolveMacOSHomebrewPythonExecutable/)
assert.match(ocrDependencySource, /resolve: resolveDefaultBasePythonExecutable/)
assert.match(ocrDependencySource, /function resolvePlatformBasePythonExecutable\(platform: NodeJS\.Platform \| string = process\.platform\)/)
...
assert.match(ocrDependencySource, /return resolvePlatformBasePythonExecutable\(\)/)
assert.doesNotMatch(ocrDependencySource, /process\.platform === 'win32'\s*\?\s*path\.join\('Scripts', 'python\.exe'\)\s*:\s*path\.join\('bin', 'python'\)/)
assert.doesNotMatch(ocrDependencySource, /if \(process\.platform !== 'win32'\) return null/)
assert.doesNotMatch(ocrDependencySource, /if \(process\.platform === 'win32'\)/)
assert.doesNotMatch(ocrDependencySource, /if \(process\.platform === "darwin"\)/)
