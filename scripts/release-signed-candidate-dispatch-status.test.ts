...
assert.match(source, /getReleaseSigningEnvironmentPlatformStatus\(value, platform\)/)
assert.match(source, /releaseTargetsMatch\(value, \{ platform, arch \}\)/)
...
assert.doesNotMatch(source, /value\.environments\.find/)
assert.doesNotMatch(source, /value\.platform !== platform|value\.arch !== arch/)
