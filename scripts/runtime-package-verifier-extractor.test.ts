...
const downloadPlan = downloader.createPlan(entry, source, 'dist-temp/tests/runtime-packages/ai-worker-core.zip')
...
const extractRoot = path.resolve('dist-temp/tests/runtime-extract')
...
const escapePlan = extractor.createPlan(entry.id, downloadPlan.destinationPath, extractRoot, path.resolve('dist-temp/tests/outside-runtime'))
...
assert.match(doc, /side-effect-free/)
