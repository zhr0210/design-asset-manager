...
stderrRunner.simulateStderr(stderrStarted.state.pid, `${process.cwd()}/ai-service/app.py failed under ${process.env.HOME}/runtime`)
...
assert.doesNotMatch(stderrState.lastError ?? '', new RegExp(process.cwd()))
if (process.env.HOME) assert.doesNotMatch(stderrState.lastError ?? '', new RegExp(process.env.HOME))
