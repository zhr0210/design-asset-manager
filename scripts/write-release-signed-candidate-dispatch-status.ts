...
const brandingEvidencePath = path.resolve(
  options.branding ?? path.join(distDir, `release-branding-evidence-${platform}-${arch}.json`)
)
const outputPath = path.resolve(
  options.output ?? path.join(distDir, `release-signed-candidate-dispatch-status-${platform}-${arch}.json`)
)
